...
# Modified Alan Emanuel Dec 2018
...
def read_data(filename):
    ...
    if data_present:
        ...
        data['amplifier_data'] = np.zeros([header['num_amplifier_channels'], num_amplifier_samples], dtype=int)
        data['aux_input_data'] = np.zeros([header['num_aux_input_channels'], num_aux_input_samples], dtype=int)
        data['supply_voltage_data'] = np.zeros([header['num_supply_voltage_channels'], num_supply_voltage_samples], dtype=int)
        data['temp_sensor_data'] = np.zeros([header['num_temp_sensor_channels'], num_supply_voltage_samples], dtype=int)
        data['board_adc_data'] = np.zeros([header['num_board_adc_channels'], num_board_adc_samples], dtype=int)
        ...
        data['board_dig_in_raw'] = np.zeros(num_board_dig_in_samples, dtype=int)

        data['board_dig_out_data'] = np.zeros([header['num_board_dig_out_channels'], num_board_dig_out_samples], dtype=bool)
        data['board_dig_out_raw'] = np.zeros(num_board_dig_out_samples, dtype=int)
    ...
    if (data_present):
        ...
        # same notch filter to amplifier data here.   Commented out by AE; do not want to apply notch filter, whether applied during recording or not
        if header['notch_filter_frequency'] > 0:
            print('Notch filter implemented during recording, not applying it now...')
